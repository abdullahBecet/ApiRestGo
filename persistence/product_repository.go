...
func (productRepository *ProductRepository) GetAllProducts() []domain.Product {
	...
	return extractProdctsFromRows(productRows)
}
...
func (productRepository *ProductRepository) AddProducts(product domain.Product) error {
	ctx := context.Background()
	...
	if err != nil {
		log.Error("Failied to Add new product", err)
		return err
	}
	log.Info(fmt.Println("Product added with %v", AddNewProduct))
	return nil

}

func extractProdctsFromRows(productRows pgx.Rows) []domain.Product {
...
func (productRepository *ProductRepository) DeleteById(productId int64) error {
	...
	deleteSql := `Delete from product where id =$1`
...
func (productRepository *ProductRepository) UpdatePrice(productId int64, newPrice float32) error {
	...
	if err != nil {
		return errors.New(fmt.Sprintf("Erorr while updateing product with id: %d", productId))
	}
